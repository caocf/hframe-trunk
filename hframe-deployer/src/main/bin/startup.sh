...
hframe_OPTS="-DappName=hframe-manager -Ddubbo.application.logger=slf4j -Dlogback.configurationFile=$logback_configurationFile -Dhframe.conf=$hframe_conf"
...
then 
	...
	$JAVA $JAVA_OPTS $JAVA_DEBUG_OPT $hframe_OPTS -classpath .:$CLASSPATH com.hframe.deployer.webcontaner.HFrameworkBootstrap 1>>$base/logs/manager.log 2>&1 &
