...
if (typeof(obj)!="object")
 ...
 t.loadXMLString("<tree id='0'>"+cont+"</tree>");window[id]=t;
 eval(""+id+" = t;");
 eval("alert("+id+");");
 return t;};function dhx_init_trees(){var z=document.getElementsByTagName("div");for (var i=0;i<z.length;i++)if (z[i].className=="dhtmlxTree")dhtmlXTreeFromHTML(z[i])
};function dhx_li2trees(tag,data,ind){for (var i=0;i<tag.childNodes.length;i++){var z=tag.childNodes[i];if ((z.nodeType==1)&&(z.tagName.toLowerCase()=="li")){var c="";var ul=null;var check=z.getAttribute("checked");for (var j=0;j<z.childNodes.length;j++){var zc=z.childNodes[j];if (zc.nodeType==3)c+=zc.data;else if (zc.tagName.toLowerCase()!="ul") c+=zc.outerHTML?zc.outerHTML:zc.innerHTML;else ul=zc;};data[data.length]=[ind,c,check];if (ul)data=dhx_li2trees(ul,data,data.length);};};return data;};if (window.addEventListener)window.addEventListener("load",dhx_init_trees,false);else if (window.attachEvent)window.attachEvent("onload",dhx_init_trees);
